...
class ControllerMarketplaceModification extends Controller {
	...
	public function refresh($data = array()) {
		...
		if ($this->validate()) {
			...
			$files = glob(DIR_SYSTEM . '*.sysmod.xml');
			...
			// Log
			$sysmod = new Log('sysmod.log');
			$sysmod->write(implode("\n", $log));
			...
			foreach ($modification as $key => $value) {
				...
				if ($original[$key] != $value) {
					...
					$log[] = 
					$sysmod->write('open'.DIR_MODIFICATION . $key.'\n');
	...
	public function clearlog() {
		...
		if ($this->validate()) {
			$handle = fopen(DIR_LOGS . 'sysmod.log', 'w+');

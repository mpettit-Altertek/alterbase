<?php
class ControllerExtensionModuleMRPbom extends Controller
{
	private $code = 'MRPbom';
	private $error = array();
	...
	public function index(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_extension_MRP_MRPbom->modifyTable($this->request->post['fields']);
	...
	// DATA
		...
		$data['fields'] = $this->model_extension_MRP_MRPbom->getTableCustomColumns();
	...
	public function install(){
		...
		$this->model_extension_MRP_MRPbom->install();
		
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSetting('module_MRPbom', ['module_MRPbom_status' => 1]);
	}

	public function uninstall(){
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_MRPbom');
		
		$this->load->model('extension/MRP/MRPbom');
		$this->model_extension_MRP_MRPbom->uninstall();
	}

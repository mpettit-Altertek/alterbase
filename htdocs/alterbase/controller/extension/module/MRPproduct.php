...
class ControllerExtensionModuleMRPproduct extends Controller
{
	...
	public function index(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_extension_MRP_MRPproduct->modifyTable($this->request->post['fields']);
	...
	// DATA
		...
		$data['types'][2] = array('name'=>'Number',		'optionsneeded'=>true	);
	...
	public function install(){
		...
		$this->model_extension_MRP_MRPproduct->install();
		
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSetting('module_MRPproduct', ['module_MRPproduct_status' => 1]);
	}

	public function uninstall(){
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_MRPproduct');
		
		$this->load->model('extension/MRP/MRPproduct');
		$this->model_extension_MRP_MRPproduct->uninstall();
	}

...
class ControllerExtensionMRPstocklocBS extends Controller
{
	private $error = array();
	...
	public function index(){
		...
		$title = '<i class="fa '.$this->language->get('heading_icon').'"></i>&nbsp;'.'MRP stock on Build Sheet report';
	...
	// BREADCRUMBS
		...
		$data['breadcrumbs'][] = array(	'text' => $title,									'href' => $this->url->link('extension/MRP/stocklocBS',	'user_token=' . $this->session->data['user_token'], true)	);
	// DATA
		$data['ajax_route'] = 'index.php?route=extension/MRP/stocklocBS/getTable&user_token=' . $this->session->data['user_token'];
		$data['type']		= 'Build Sheet';
	...
	public function getTable(){
		...
		$data = $this->model_extension_MRP_MRPstock->get_stock_history_for_loc(7,$this->request->get,$columns);
